...
# CONFIGURATION
# ==========================================
EC2_HOST="ec2-13-63-127-117.eu-north-1.compute.amazonaws.com"
EC2_USER="ubuntu"
PEM_KEY_PATH="/home/bosenilotpal/archaser.pem"
REMOTE_APP_DIR="/home/ubuntu/production"
BUILD_DIR="frontend/build"
PM2_APP_NAME="archaser-main"
...
LINUX_BUILD_DIR="$HOME/.archaser-build-production"
...
echo -e "${CYAN}Starting PRODUCTION deployment to $EC2_HOST...${NC}"
...
if [ $START_STEP -le 1 ]; then
    echo -e "\n${YELLOW}[1/4] Building project locally (Production)...${NC}"
    build_start=$(date +%s)
    ...
    # Copy .env.production as .env.production.local for the build
    if [ -f "$SRC_DIR/frontend/.env.production" ]; then
        echo -e "-> Using .env.production for build..."
        cp "$SRC_DIR/frontend/.env.production" "$LINUX_BUILD_DIR/.env.production.local"
    else
...
# 2. Package Artifacts
artifact_name="deploy-prod.tar.gz"
...
# 4. Remote Deployment (Mapped to Step 4)
if [ $START_STEP -le 4 ]; then
    ...
    # Rename .env.production to .env so pm2/next picks it up automatically
    cp $REMOTE_APP_DIR/frontend/.env.production $REMOTE_APP_DIR/frontend/.env 2>/dev/null || true
    cp $REMOTE_APP_DIR/backend/.env.production $REMOTE_APP_DIR/backend/.env 2>/dev/null || true
    # Prefer backend env at deploy root for compose/PM2 tooling that still reads ./.env
    cp $REMOTE_APP_DIR/backend/.env.production $REMOTE_APP_DIR/.env 2>/dev/null || true
    ...
    echo -e "\n${GREEN}Production deployment finished successfully!${NC}"
else

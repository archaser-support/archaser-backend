...
export function isEligibleForCustomerMepOverdue(
    amount: number | null | undefined
): boolean {
...
/**
 * Snapshot at import: true ⇒ ctv_customer_overdue_mep.
 *
 * Callers must pass the MEP block state **as of the invoice's own issue date**.
 * `Customer.overdue_block` is a wall-clock column, so passing it directly stamps
 * whatever the block happened to be when the row was written (a chronological
 * replay then reflects its own progress instead of each invoice's creation day).
 */
export function computeCreatedTermsViolationCustomerOverdueMep(
    customerOverdueBlockAtInvoiceDate: boolean | null | undefined
): boolean {
    return customerOverdueBlockAtInvoiceDate === true;
}
...
export function computeCreatedTermsViolationSnapshot(args: {
    invoice_date: Date;
    /** Credit notes never drive customer MEP, so they cannot be created in MEP violation. */
    invoice_amount?: number | null;
    /** MEP block evaluated on {@link invoice_date}; falls back to the live column when absent. */
    customer_overdue_mep_at_invoice_date?: boolean | null;
    /** Account MEP breach start date; invoices issued before it are never flagged. */
    ...
    isInvoiceInMepBreachScope(args.invoice_date, args.mep_breach_start_date)
        ? computeCreatedTermsViolationCustomerOverdueMep(
              args.customer_overdue_mep_at_invoice_date ??
                  args.customer.overdue_block
          )
        : false;
 ...
 */
export function computeInsuranceTargetDates(args: {
    amount?: number | null;
    ...
    return {
        target_reporting_date: computeTargetReportingDate(
            args.due_date,
...
export function computeInvoiceInsuranceRowData(args: {
    ...
    const { target_reporting_date, target_mep_date } =

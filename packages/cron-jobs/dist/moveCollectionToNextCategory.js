...
exports.moveCollectionToNextCategory = moveCollectionToNextCategory;
const createCategoryChangeActivity_1 = require("./activities/createCategoryChangeActivity");
const invalidateDashboardCacheForAccounts_1 = require("./dashboard/invalidateDashboardCacheForAccounts");
...
async function moveCollectionToNextCategory(prisma, freeze) {
    ...
    try {
        ...
        for (const collection of expiredCollections) {
            try {
                const nextCategory = collection.previous_category || "Automated";
        ...
        for (const collection of collections) {
            if (collection.current_category !== "Automated" ||
            ...
            if (collection.current_category === "Automated" &&
                ...
                if (waitDays > 0 && collection.next_category_date) {
                    ...
                    const timeSinceNextCategoryDate = now.getTime() - collection.next_category_date.getTime();
        ...
        for (const collection of filteredCollections) {
            try {
                await updateCollectionPeriodCategorySlim(prisma, collection.id, collection.next_category, collection.current_category, collection.Customer.account_id, collection.Customer.id);
        ...
        try {
            const accountIds = Array.from(new Set(filteredCollections.map((c) => c.Customer.account_id)));
            if (accountIds.length > 0) {
                await (0, invalidateDashboardCacheForAccounts_1.invalidateDashboardCacheForAccounts)(prisma, accountIds);
            }
        }
        catch {
        ...
        if (summary.phase2.collectionsProcessed > 0) {
            await (0, publishControlCenterUpdate_1.publishControlCenterUpdate)(`moveCollectionToNextCategory: processed ${summary.phase2.collectionsProcessed} collection(s)`, {
...
async function updateCollectionPeriodCategorySlim(prisma, collectionId, nextCategory, currentCategory, accountId, customerId) {
    ...
    let activitiesToCancel = null;
    if (currentCategory === "Automated" ||
    ...
    let promiseActivitiesToDelete = null;
    if (currentCategory === "Promise_to_pay") {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        await (0, createCategoryChangeActivity_1.createCategoryChangeActivity)(tx, {
            customerId,
            collectionId,
            accountId,
            currentCategory,

 ...
 * Phase 1: Send due SCHEDULED activities
 */
async function sendDueActivities(prisma, options, stats) {
    const now = new Date();
 ...
 */
async function processSmsActivity(prisma, activity, pendingContacts, options, stats) {
 ...
 */
async function resolveSmsVendor(prisma, accountId, countryId) {
    // Try country-specific vendor first via AccountSMSProviderPreferences
    if (countryId) {
        const preference = await prisma.accountSMSProviderPreferences.findFirst({
            where: {
    ...
    const defaultPreference = await prisma.accountSMSProviderPreferences.findFirst({
        where: {
            account_id: accountId,
            is_enabled: true,
        },
...
async function generateNextActivities(prisma, options, stats) {
    ...
    const periodsNeedingActivities = await prisma.customerCollectionPeriod.findMany({
        where: whereClause,
        include: {
            Customer: {
                select: {
    ...
    const accountIds = Array.from(new Set(periodsNeedingActivities
    ...
    const sequenceKeys = periodsNeedingActivities.map((p) => {
        ...
        const containerId = customer?.sequence_container_id ??
    ...
    const uniqueSequenceKeys = Array.from(new Map(sequenceKeys.map((k) => [
        `${k.accountId}_${k.containerId}`,
        k,
    ])).values());
    const sequencesRaw = await Promise.all(uniqueSequenceKeys.map((key) => prisma.activitiesSequence.findMany({
        where: {
    ...
    for (const period of periodsNeedingActivities) {
        try {
            await processCollectionPeriodForNextActivity(prisma, period, sequencesByKey, defaultContainerByAccount, stats);
        }
        catch (err) {
            const error = err;
 ...
 */
async function processCollectionPeriodForNextActivity(prisma, period, sequencesByKey, defaultContainerByAccount, stats) {
    ...
    const contacts = filterContactsBySequence(allContacts, {
        send_to_standard_contacts: nextSequence.send_to_standard_contacts ?? false,
        send_to_escalated_contacts: nextSequence.send_to_escalated_contacts ?? false,
    });

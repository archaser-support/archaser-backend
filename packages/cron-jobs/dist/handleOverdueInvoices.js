...
async function createOpenCollectionPeriods(prisma, customerData) {
    let created = 0;
    let skippedCreditOnly = 0;
    (0, credit_insurance_domain_1.bindCreditInsurancePrisma)(prisma);
    for (const data of customerData) {
        const isCreditOnly = data.hasCollection === false && data.hasCreditInsurance === true;
        if (isCreditOnly) {
            skippedCreditOnly += 1;
            continue;
        }
        ...
        const categoryToUse = data.categoryForNew ||
        ...
        await prisma.customerCollectionPeriod.create({
            data: {
                ...
                total_outstanding_amount: data.amounts.total_outstanding_amount || 0,
                customer_outstanding_amount1: data.amounts.customer_outstanding_amount1 || 0,
                customer_outstanding_amount2: data.amounts.customer_outstanding_amount2 || 0,
                current_category: categoryToUse,
                last_automated_step: 0,
                create_next_activity: true,
            },
        });
        await (0, credit_insurance_domain_1.syncCustomerInsuranceFields)(data.customerId);
        created += 1;
    }
...
async function handleOverdueInvoices(prisma, scope, freeze) {
    ...
    if (pastDueInvoices.length > 0) {
        ...
        affectedCustomerIds = Array.from(new Set(pastDueInvoices
            .map((i) => i.customer_id)
        ...
        (0, credit_insurance_domain_1.bindCreditInsurancePrisma)(prisma);
        await (0, credit_insurance_domain_1.sweepReportingBreachForOverdueInvoiceIds)(invoiceIds, prisma);
    }
    // Refresh oldest overdue / overdue_block for open periods
    {
        ...
        const uniqueCustomerIds = Array.from(new Set(openPeriods.map((p) => p.customer_id)));
        for (const cid of uniqueCustomerIds) {
            await (0, credit_insurance_domain_1.syncCustomerInsuranceFields)(cid);
        }
    ...
    const outstandingMap = await (0, customersDomain_1.calculateOutstandingAmountsForCustomersViaApi)(affectedCustomerIds, prisma);
    ...
    const inactiveCustomerIds = new Set(customers
        .filter((c) => c.collection_status === "Inactive")
        .map((c) => c.id));
    const openPeriodCustomerIds = new Set(openPeriodRows.map((r) => r.customer_id));
    ...
    const customersNeedingCollectionPeriod = affectedCustomerIds.filter((id) => {
        const amounts = outstandingMap.get(id);
        if (!amounts)
            return false;
        if ((amounts.no_of_overdue_invoices ?? 0) <= 0)
            return false;
        if ((amounts.total_outstanding_amount ?? 0) <= 0)
            return false;
        if (openPeriodCustomerIds.has(id))
            return false;
        return true;
    });
    if (customersNeedingCollectionPeriod.length > 0) {
        ...
        const payload = withInvoices.map((customer) => {
            ...
            const oldestOverdueDate = customer.Invoice.length > 0 && customer.Invoice[0]?.due_date
            ...
            return {
                ...
                accountCategoryForNew: base?.Account?.category_for_new_collection ?? null,
                hasCollection: base?.Account?.has_collection ?? null,
                hasCreditInsurance: base?.Account?.has_credit_insurance ?? null,
                oldestOverdueDate,
            };
        });

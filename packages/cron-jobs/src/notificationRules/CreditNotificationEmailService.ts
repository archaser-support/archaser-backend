...
import { replaceDoubleBraceTemplateVariables } from "../templates/processTemplateContent";
...
async function loadCreditAlertTemplate(
    prisma: PrismaClient,
    accountId: number
) {
...
export class CreditNotificationEmailService {
    ...
    async sendCreditAlertEmail(input: {
        ...
        const template = await loadCreditAlertTemplate(
            this.prisma,
            input.accountId
        );
        ...
        if (template) {
            emailSubject = replaceDoubleBraceTemplateVariables(
                template.subject,
                variables
            );
            emailBody = replaceDoubleBraceTemplateVariables(
                template.content,
                variables
            );
        } else {

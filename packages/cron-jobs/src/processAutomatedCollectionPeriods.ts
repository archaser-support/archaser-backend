...
export async function processAutomatedCollectionPeriods(
    ...
    try {
        ...
        const activityAccountIds = Array.from(
            new Set(
                activities
        ...
        const activityMaxSteps =
            activityAccountIds.length > 0
                ? await prisma.activitiesSequence.groupBy({
                      by: ["account_id", "sequence_container_id"],
                      where: {
        ...
        const activityMaxStepMap = new Map<string, number>();
        ...
        if (transitionPeriods.length > 0) {
            ...
            const transitionAccountIds = Array.from(
                new Set(
                    transitionPeriods
                        .map((p) => p.Customer?.account_id)
                        .filter((id): id is number => typeof id === "number")
            ...
            const transitionMaxSteps =
                transitionAccountIds.length > 0
                    ? await prisma.activitiesSequence.groupBy({
                          by: ["account_id", "sequence_container_id"],
                          where: {
            ...
            const transitionMaxStepMap = new Map<string, number>();
            ...
            const latestTransitionActivities = new Map<
                number,
                (typeof transitionActivities)[0]
            >();
            for (const activity of transitionActivities) {
                if (
                    activity.collection_period_id &&
                    !latestTransitionActivities.has(
                        activity.collection_period_id
                    )
                ) {
                    latestTransitionActivities.set(
                        activity.collection_period_id,
                        activity
                    );
                }
            ...
            for (const period of transitionPeriods) {
                ...
                const nextCategoryDate = new Date(
                    deliveryTime.getTime() + waitDays * 24 * 60 * 60 * 1000
                );

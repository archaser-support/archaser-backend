...
export type ExtensionEntityType = "Customer" | "Payment" | "Invoice" | "Contact";
/** Cross-entity mapped rows for one sync window. */
export type ExtensionMappedBatch = Partial<Record<ExtensionEntityType, Record<string, unknown>[]>>;
...
export type ExtensionAfterPaymentLinkedContext = {
    prisma: Pick<PrismaClient, "invoice" | "invoicePayment" | "billingConnector" | "$transaction">;
    accountId: number;
    userId?: string;
...
export interface BillingAccountExtension {
     ...
     */
    transform(ctx: ExtensionTransformContext): ExtensionMappedBatch | Promise<ExtensionMappedBatch>;
    /**
     ...
     */
    shouldNormalizeNegativeCreditPayments?(row: ExtensionCreditPaymentCloseInput): boolean;

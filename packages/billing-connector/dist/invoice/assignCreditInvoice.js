...
exports.assignCreditInvoice = assignCreditInvoice;
async function assignCreditInvoice(prisma, assignment) {
    ...
    const currentCustomerNetAmount = currentTargetInvoice.customer_net_amount || 0;
    const currentTotalPaid = currentTargetInvoice.total_paid || 0;
    const newCustomerNetAmount = Math.max(0, currentCustomerNetAmount - creditAmount);
    ...
    if (originalCustomerAmount > 0) {
        ...
        const ratio = originalAmount /
            (currentTargetInvoice.net_amount || originalAmount);
        newNetAmount = newCustomerNetAmount * ratio;
    }
    else if (currentCustomerNetAmount > 0) {
        const reductionRatio = newCustomerNetAmount / currentCustomerNetAmount;
        newNetAmount = (currentTargetInvoice.net_amount || 0) * reductionRatio;
    }
    const newOutstandingDebt = newNetAmount - currentTotalPaid;
    const currentCustomerTotalPaid = currentTargetInvoice.customer_total_paid || 0;
    const newCustomerOutstandingDebt = Math.max(0, newCustomerNetAmount - currentCustomerTotalPaid);

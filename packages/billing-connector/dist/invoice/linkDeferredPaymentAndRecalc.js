...
exports.resolveInvoicePaidTolerance = exports.normalizeInvoicePaidTolerance = exports.isWithinPaidTolerance = exports.INVOICE_PAID_TOLERANCE = void 0;
exports.resolveInvoicePaidRecalcOptions = resolveInvoicePaidRecalcOptions;
exports.bulkLinkDeferredPayments = bulkLinkDeferredPayments;
...
exports.linkDeferredPaymentsAndRecalcBatch = linkDeferredPaymentsAndRecalcBatch;
 ...
 */
async function resolveInvoicePaidRecalcOptions(prisma, accountId, overrides) {
    const [isForcePaidClose, paidTolerance] = await Promise.all([
        ...
        overrides?.paidTolerance != null
            ...
            : (0, invoicePaidTolerance_1.resolveInvoicePaidTolerance)(prisma, accountId),
    ]);
    return {
        ...overrides,
        isForcePaidClose,
...
function buildInvoicePaidUpdate(invoice, linkedPayments, options, modifiedAt, paidTolerance) {
    ...
    const becomesPaid = (0, invoicePaidTolerance_1.isWithinPaidTolerance)(newCustomerOutstanding, paidTolerance);
    return {
        total_paid: totalPaid,
        customer_total_paid: totalCustomerPaid,
...
function toInvoicePaidRecalcRow(invoice, linkedPayments, options, paidTolerance) {
    const update = buildInvoicePaidUpdate(invoice, linkedPayments, options, new Date(), paidTolerance);
    ...
    return {
        ...
        customer_total_paid: update.customer_total_paid,
        outstanding_debt: update.outstanding_debt,
        customer_outstanding_debt: update.customer_outstanding_debt,
        status,
        clearAlerts: update.zero_limit_alert === false &&
            update.reporting_breach === false,
    };
}
async function bulkWriteInvoicePaidRecalcRows(prisma, rows, modifiedAt) {
    if (rows.length === 0) {
        return;
    }
    for (let i = 0; i < rows.length; i += RECALC_PROGRESS_CHUNK) {
        ...
        const customerOutstandingDebt = chunk.map((row) => row.customer_outstanding_debt);
 ...
 */
async function bulkLinkDeferredPayments(prisma, accountId, rows, modifiedAt, options) {
    if (rows.length === 0) {
        return 0;
    }
    let linked = 0;
    for (let i = 0; i < rows.length; i += BULK_PAYMENT_LINK_CHUNK) {
        ...
        const simple = chunk.filter((row) => row.customer_currency === undefined);
        const aligned = chunk.filter((row) => row.customer_currency !== undefined);
        let chunkLinked = 0;
        if (simple.length > 0) {
        ...
        if (aligned.length > 0) {
            ...
            const customerAmounts = aligned.map((row) => row.customer_amount ?? 0);
            const currencies = aligned.map((row) => row.customer_currency ?? "");
            const result = await prisma.$executeRaw `
                UPDATE "InvoicePayment" AS p
                SET
                    invoice_id = data.invoice_id,
...
async function recalculateInvoiceFromLinkedPayments(tx, invoiceId, options) {
    ...
    const paidTolerance = options?.paidTolerance ??
        (await (0, invoicePaidTolerance_1.resolveInvoicePaidTolerance)(tx, invoice.account_id));
    return tx.invoice.update({
        where: { id: invoiceId },
        data: buildInvoicePaidUpdate(invoice, linkedPayments, { ...options, isForcePaidClose }, new Date(), paidTolerance),
    });
}
/**
...
async function recalculateInvoicesFromLinkedPayments(prisma, targets, options) {
    ...
    const paidToleranceByAccount = new Map();
    for (const invoice of invoices) {
        ...
        if (!paidToleranceByAccount.has(invoice.account_id)) {
            paidToleranceByAccount.set(invoice.account_id, await (0, invoicePaidTolerance_1.resolveInvoicePaidTolerance)(prisma, invoice.account_id));
        }
    }
    const modifiedAt = new Date();
    const recalcRows = invoices.map((invoice) => toInvoicePaidRecalcRow(invoice, paymentsByInvoiceId.get(invoice.id) ?? [], {
    ...
    if (!options?.onProgress) {
        await bulkWriteInvoicePaidRecalcRows(prisma, recalcRows, modifiedAt);
        return;
    }
    options.onProgress({ processed: 0, total: recalcRows.length });
    for (let offset = 0; offset < recalcRows.length; offset += RECALC_PROGRESS_CHUNK) {
        const chunk = recalcRows.slice(offset, offset + RECALC_PROGRESS_CHUNK);
        await bulkWriteInvoicePaidRecalcRows(prisma, chunk, modifiedAt);
        options.onProgress({
 ...
 */
async function linkDeferredPaymentsAndRecalcBatch(prisma, links, recalcOptions) {
    if (links.length === 0) {
    ...
    for (const link of links) {
        ...
        if (!payment)
            continue;
        if (payment.invoice_id === link.invoiceId)
            continue;
        if (payment.invoice_id != null)
            continue;
        pending.push({
    ...
    const paymentsLinked = await bulkLinkDeferredPayments(prisma, accountId, pending.map((row) => ({
...
async function linkDeferredPaymentAndRecalc(prisma, params) {
    ...
    if (recalcOptions?.paidTolerance == null) {
        ...
        recalcOptions = await resolveInvoicePaidRecalcOptions(prisma, paymentAccount.account_id, recalcOptions);
    }
    return prisma.$transaction(async (tx) => {
        ...
        if (payment.invoice_id === invoiceId) {
            ...
            const updatedInvoice = await recalculateInvoiceFromLinkedPayments(tx, invoiceId, recalcOptions);
            return {
                invoicePayment: payment,
                updatedInvoice,
                alreadyLinked: true,

...
exports.linkOrphanedCreditNotes = linkOrphanedCreditNotes;
const assignCreditInvoice_1 = require("./assignCreditInvoice");
async function linkOrphanedCreditNotes(prisma, params) {
    const { accountId, targetInvoiceNumbers } = params;
    const uniqueNumbers = Array.from(new Set(targetInvoiceNumbers.filter((n) => Boolean(n?.trim()))));
    ...
    for (const creditNote of orphanedCreditNotes) {
        if (!creditNote.credit_for_invoice_number)
            continue;
        const targetInvoice = targetByNumber.get(creditNote.credit_for_invoice_number);
        if (!targetInvoice)
            continue;
        const rawCreditAmount = Math.abs(creditNote.customer_net_amount ?? 0) ||
        ...
        if (rawCreditAmount <= 0)
            continue;
        try {
            await (0, assignCreditInvoice_1.assignCreditInvoice)(prisma, {
            ...
            console.error(`Failed to link orphaned credit note ${creditNote.id} to target ${targetInvoice.id}:`, error);
        }
    }

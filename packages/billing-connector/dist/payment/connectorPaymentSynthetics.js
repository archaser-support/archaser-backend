...
exports.PAYMENT_SYNTHETIC_FIELDS = void 0;
exports.buildPaymentReference = buildPaymentReference;
exports.collectPaymentReferenceAliases = collectPaymentReferenceAliases;
exports.applyPaymentSynthetics = applyPaymentSynthetics;
exports.applyPaymentSyntheticsToRecords = applyPaymentSyntheticsToRecords;
...
function withOptionalKline(base, kline) {
    const k = asTrimmedString(kline);
    return k ? `${base}|${k}` : base;
}
function pickNonZeroAmount(row) {
 ...
 */
function buildPaymentReference(row) {
 ...
 */
function collectPaymentReferenceAliases(row, mappedReference, invoiceNumber) {
...
function pickPaymentInvoiceNumber(row) {
    const fnciref1 = asTrimmedString(row.FNCIREF1);
    if (fnciref1) {
        return fnciref1;
    }
...
exports.PAYMENT_SYNTHETIC_FIELDS = [
    "PAY_AMOUNT",
    "PAY_DATE",
    "PAY_REFERENCE",
    "PAY_INVOICE_NUMBER",
 ...
 */
function applyPaymentSynthetics(row) {
    const next = { ...row };
...
function applyPaymentSyntheticsToRecords(records) {

...
overview: "Enforce mandatory invoice/payment fields on billing-connector live sync only: skip incomplete rows (never insert or overwrite with defaults), keep good stored data on incomplete re-pulls, surface skips via Loki + import_validation metrics + PARTIAL sync status, and make failed/skipped imports visible in Grafana (staging) and the Billing sync history grid."
todos:
  - id: shared-validator
  ...
  - id: frontend-sync-history
    ...
    status: pending
  - id: preview-catalog
    content: "Align getImportEntityFieldCatalog requiredFields with live rules (due_date, currency)"
    status: pending
isProject: false
---
...
flowchart TB
  ...
  Stats --> Finish[PARTIAL + import_validation on finish]
...
### `sample_errors` aggregation (V3–V4, V7)

Shared helper `appendEntityImportIssue(slice, message)` used from invoice batch, payment import, and sync loops:

- **Inputs:** pooled messages from **failed** and **skipped** import rows for that entity type in the current sync run (accumulated across pages).
- **Message format:** `{identifier}: {reason}` — e.g. `INV-88: missing due_date`, `PAY-42: missing invoice_number`, `INV-9: Customer CUST-9 not found`.
...
| `packages/billing-connector/src/sync/runInProcessSync.ts` | `enforceMandatoryFields: true`; accumulate `importResult.failed` / `.skipped` per entity into running stats; pass `sample_errors` to progress + finish |
| `packages/billing-connector/src/sync/stagedExtensionSync.ts` | Same accumulation on extension import path |
...
| `packages/billing-connector/src/observability/types.ts` | `mandatoryFieldSkips?: number` on sync stats |
| `packages/billing-connector/src/observability/statusAndErrorType.ts` | PARTIAL / `import_validation` when mandatory skips or import row issues |
| `packages/billing-connector/src/observability/emitSyncObservability.ts` | `records_processed_total` for `failed` + `skipped`; `import_validation` on finish when issues > 0 |
| `packages/billing-connector/src/utils/connectorFieldUtils.ts` | Preview catalog: add `due_date`, `currency` to Invoice `requiredFields` |
...
| `runPreviewSync.ts` | Catalog alignment sufficient for preview |
| Deferred payment maturity | Unchanged (D11) |
| `api/src/billing-connector/billing-connector.service.ts` | Finish observability from shared `runInProcessSync` path |
...
2. On fail: `skipped++`, record `formatImportIssueMessage(invoice_number, reason)` via aggregator; do not insert/update.
...
1. Validate before prepare loop; on fail → `skipped: true` + issue message.
...
| `skipped` | `importResult.skipped` (accumulated) |
| `sample_errors` | Pooled failed + skipped messages via aggregator (V3, V7) |

Fix `entityStatsFromCounts` to use these values instead of hard-coded `failed: 0`, `skipped: 0`.

**Sync finish semantics:**

| Scenario | Status | `error_type` |
|----------|--------|--------------|
| All entities succeed, 0 issues | SUCCESS | null |
| Some imported, some skips/failures | PARTIAL | `import_validation` |
| 0 imported, all skips/failures | FAILED | `import_validation` |
| 0 imported, pull/connection errors | FAILED | per existing classification |

### Phase 5 — Observability (D8 + V1)

**On sync finish** (`billing_connector.sync` JSON line):

- `entity_stats` string includes per-entity `failed`, `skipped`, `sample_errors`
- `emitBillingConnectorSyncFinish` increments `errors_total{error_type="import_validation"}` when any entity has `failed > 0`, `skipped > 0`, or `mandatoryFieldSkips > 0`
- `records_processed_total{result="failed"}` and `{result="skipped"}` increment from fixed entity stats (powers V5 stat panel)
...
### Phase 6 — Preview catalog alignment

Update `getImportEntityFieldCatalog`: Invoice `requiredFields` += `due_date`, `currency`; Payment keeps `payment_date`, `customer_currency`.
...
| R12 | Mandatory skips → PARTIAL + `import_validation` | `statusAndErrorType` | `billing-connector` |
| R13 | `entity_stats.failed` / `skipped` populated | `entityStatsFromCounts` | `billing-connector` |
...
| Risk | Mitigation |
|------|------------|
| High skip volume on bad mapping | Preview + `sample_errors` + skipped count; fix mapping |
| `skipped` mixes duplicate-payment skips with mandatory skips | Both are valid “row not applied”; `sample_errors` distinguishes mandatory vs duplicate if message differs |
| Loki panel JSON parse fragility | Filter on `status=~"PARTIAL|FAILED"` first; document expand-log workflow |
| Watermark advances despite skips | `import_validation` + `advanceWatermark: true` — intentional |
| Extension staged sync path missed | Both `runInProcessSync` and `stagedExtensionSync` listed in Phase 4 |

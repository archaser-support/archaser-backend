...
- [ ] Card shows idle named count, ratio vs named-in-range denominator, and assessment cost
- [ ] Idle = open AR $0 on every named day in range (snapshot/open-AR based, not invoice document dates)
- [ ] Assessment cost uses current live fee × idle count × shared year multiplier (multi-policy Σ)
- [ ] Null fee → $0 cost but count/ratio still show
- [ ] No new click-through report

...
function formatAmountWithoutSymbol(amount, locale = "en-US") {
...
function calculateRelevanceScore(primaryText, searchTerm, secondaryText) {
    if (!primaryText)
        return 0;
    ...
    let score = 0;
    if (primaryLower === searchLower)
        score += 100;
    else if (primaryLower.startsWith(searchLower))
        score += 80;
    else if (primaryLower.includes(searchLower))
        score += 50;
    if (secondaryText) {
        if (secondaryLower === searchLower)
            score += 30;
        else if (secondaryLower.includes(searchLower))
            score += 15;
    }
...
let SearchService = class SearchService {
    ...
    async globalSearch(user, searchTerm) {
        if (!searchTerm ||
        ...
        const hasViewAsPermission = await this.accessScope.hasPermission(effectiveAccountId, effectiveRole, "use_view_as");
        const ownerFilter = isSystemAdmin
            ? {}
            : await this.accessScope.getOwnerFilter(userInfo.userId, hasViewAsPermission, userInfo.viewAsUserId, userInfo.viewAsUserRole, userInfo.viewAsUserAccountId);
        const buFilter = await this.accessScope.getBusinessUnitFilter(userInfo.businessUnitId, isSystemAdmin, accountId);
        const [customers, invoices, contacts, disputes] = await Promise.all([
            this.searchCustomers(trimmedSearch, accountId, ownerFilter, buFilter),
            this.searchInvoices(trimmedSearch, accountId, ownerFilter, buFilter),
            this.searchContacts(trimmedSearch, accountId),
            this.searchDisputes(trimmedSearch, accountId, ownerFilter, buFilter),
        ]);
        const countsByType = {
        ...
        const allResults = [
            ...customers.map((c) => {
                const relevanceScore = calculateRelevanceScore(c.name, trimmedSearch, c.customer_number);
                const formattedOverdueAmount = c.total_invoices_overdue
                    ? formatAmountWithoutSymbol(Number(c.total_invoices_overdue), "en-US")
                    : null;
                return {
                    id: c.id,
                    type: "customer",
                    ...
                    metadata: {
                        ...
                        total_invoices_overdue_formatted: formattedOverdueAmount,
            ...
            ...invoices.map((inv) => {
                const relevanceScore = calculateRelevanceScore(inv.invoice_number || "", trimmedSearch, inv.customer_name || "");
            ...
            ...contacts.map((cont) => {
                const relevanceScore = calculateRelevanceScore(cont.name, trimmedSearch, cont.email || cont.phone);
                return {
                    id: cont.id,
                    type: "contact",
                    name: cont.name,
            ...
            ...disputes.map((dis) => {
                const relevanceScore = calculateRelevanceScore(dis.customer_name, trimmedSearch, dis.reason_name);
                return {
                    id: dis.id,
                    type: "dispute",
                    ...
                    metadata: {
                        ...
                        created_at: dis.created_at
                            ? new Date(dis.created_at).toLocaleDateString("en-US", {
                                year: "numeric",
                                month: "2-digit",
                                day: "2-digit",
                            })
    ...
    async searchCustomers(searchTerm, accountId, ownerFilter, buFilter) {
        const where = {
            AND: [
        ...
        return customers.map((customer) => {
            ...
            return {
                ...
                current_category: customer.CustomerCollectionPeriod?.[0]?.current_category ||
                    null,
            };
        });
    }
    async searchInvoices(searchTerm, accountId, ownerFilter, buFilter) {
        const conditions = [
            { account_id: accountId },
        ...
        return invoices.map((invoice) => ({
            ...
            customer_name: invoice.Customer?.type === "Person"
    ...
    async searchContacts(searchTerm, accountId) {
    ...
    async searchDisputes(searchTerm, accountId, ownerFilter, buFilter) {
        const disputeSearchConditions = [];
        ...
        return disputes.map((dispute) => ({
            ...
            customer_name: dispute.Customer.type === "Person"
